---
description: 'Component communication via Store.tsx — shared state, pub/sub events, and product binding patterns'
applyTo: '**'
---

# Component Communication

All cross-component state in this project flows through `src/utils/Store.tsx`. Do NOT invent ad-hoc React context providers, prop-drilling chains, or external state libraries. The Store already provides reactive key-value state, pub/sub events, and pre-built adapters for products, cart, search, and fulfillment.
...
Store.tsx has three layers. Pick the highest-level one that fits:

| Layer | What it does | When to use |
|-------|-------------|-------------|
| **Adapter hooks** | Ready-to-spread bindings for headers, product cards | Wiring WCP components to shared state |
...
Use `useStoreConnectedItemBindings()` to get a factory, then call it per product. The returned bindings spread directly onto WCP tile/card components:
...
`WCPFlashDealsCarousel` is a fixed-data organism. Its built-in `+ Add` actions write directly to `window.ldKit.items.addToCart(...)`, so header cart totals still update when the component is dropped onto a page.

Use this as a convenience for the canned flash-deals content. For **custom product rows**, always build `Carousel` + `WCPCarouselProductCard` and wire each product via `useStoreConnectedItemBindings()`.
...
<WCPHeader cartCount={myItems.length} cartPrice={formatPrice(myTotal)} />
```
...
- MUST use Store.tsx for any state shared between components. Do not create React context providers, Redux stores, Zustand stores, or other state management for cross-component communication.
...
- MUST use `useHeaderCartBindings()` when mounting `WCPHeader` — do not manually format cart totals.
...
- MUST treat `WCPFlashDealsCarousel` as fixed-data only — never invent unsupported props (for example `items`) to pass custom product arrays.
- NEVER duplicate cart/favorites/search logic that Store.tsx already provides.
              ...
              {similarProducts.map(p => {
                ...
                return (
                  <CarouselItem key={p.sku} UNSAFE_className="ld-carousel-item--multi">
                    <WCPCarouselProductCard onAddToCart={b.onAddToCart} cartQty={b.cartQty} onCartQtyChange={b.onCartQtyChange} />
                  </CarouselItem>
                );
              })}
            </CarouselContent>
