...
function computeTimerState(endTime: Date | number | string): WCPTimerState {
...
export function useWCPTimer(
  ...
  const [state, setState] = React.useState<WCPTimerState>(() => computeTimerState(endTime));
...
const BADGE_COLOR_TO_VARIANT: Record<WCPTimerBadgeColor, string> = {
  blue: 'waiting',
  spark: 'warning',
  negative: 'expiring',
};

export const WCPTimerView: React.FunctionComponent<WCPTimerViewProps> = (props) => {
...
WCPTimerView.displayName = 'WCPTimerView';
